...
library(ggsignif)       # For comparison tests' brackets
...
```{r cmx_individual_differences_gamm}
# frm <- ~
#     (0 + dummy(Condition, "zero") | idx) +
#     (0 + dummy(Condition, "coin") | idx) +
#     (0 + dummy(Condition, "stock") | idx)
# 
# gam0 <- gamm4(cmx_w ~ Condition + s(t), random = ~ (1 | idx), data = df)
# gam1 <- gamm4(cmx_w ~ Condition + s(t), random = frm, data = df)
```
...
Poniżej przedstawiona jest analiza dewiancji sprawdzająca, czy model przyzwalający na różne wariancje efektów losowych
w grupach `zero` i `coin` jest lepszy od modelu, który na to nie pozwala
(efektywnie jest to test istotności róznicy między wariancjami w grupach `zero` i `coin`).
Widzimy, że wariancje w grupach różnią się (por. wykres poniżej).

```{r cmx_individual_differences_zero_vs_coin}
lmm_zero_coin0 <- lmer(
    formula = cmx_w ~ Condition + t +
        (0 + I(dummy(Condition, "zero") + dummy(Condition, "coin")) | idx) +
        (0 + dummy(Condition, "stock") | idx),
    data = df, 
    REML = FALSE
)
...
(aov_zero_coin <- anova(lmm_zero_coin0, lmm_zero_coin1))
```
...
eff %>%
    ...
    geom_signif(comparisons = list(c("zero", "coin")), color = "gray40", linetype = 2L,
                annotation = str_glue("p = 0.0192"), tip_length = c(.4, 0.02)) +
